...
class TargetSelection:
    ...
    def selectTarget(self, init_ogm, coverage, robot_pose, origin, \
        ...
        if self.method == 'random' or force_random == True:
            ...
            Print.art_print("Select smart target time: " + str(time.time() - tinit), Print.BLUE)
            Print.art_print("Selected Target - Node: " + str(nodes[idx][2]), Print.BLUE)
            target = nodes[idx]
        else:
            Print.art_print("[ERROR] target_selector at autonomous_expl.yaml must be either 'random' or 'smart'", Print.RED)
        ########################################################################
